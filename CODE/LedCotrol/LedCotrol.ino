...
const char* mqttUser = "LedCotrol123";
...
//  digitalWrite(LED_BUILTIN, 1);     // esp8266 
  startWiFi();
  connectBroker();
  client.subscribe("Led1"); // Đăng ký nhận tin từ topic = "Led"
  client.publish("Led1", "Hello from Blocky Node"); // Gởi message = "Hello from Blocky Node" đến topic = "Led"
}
void loop() {
  if (!client.connected()) {
    connectBroker();
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (message == "on") {
    ...
    client.publish("Led1", "LED is turned ON"); // Thông báo trạng thái bật LED cho tất cả client đăng ký topic = "Led"
  }
  else if (message == "off") { // Tắt đèn LED khi nhận được tin nhắn "off"
    digitalWrite(led, LOW);
    //digitalWrite(LED_BUILTIN, 0);
    client.publish("Led1", "LED is turned OFF"); // Thông báo trạng thái tắt LED cho tất cả client đăng ký topic = "Led"
  }
}
